...
app = Flask(__name__)
app.secret_key = 'saints-and-sinners-no-kyc-engine-v1'

# =====================================================================================
# KNOWLEDGE BASE & PERSONALITY PROFILES (Senza strategie specifiche KYC)
# =====================================================================================
...
TASK_STRATEGIES = {
    # MODIFICA CHIAVE: La sezione KYC ora ha una strategia generica basata sulla filosofia, non sulle tecniche specifiche.
    "kyc_collect": {
        "name": "First Contact & Relationship Building",
        "strategy": "Your goal is to gather information (like name, location, etc.) by making the fan feel comfortable. Do not use specific interrogation techniques. Instead, build rapport with a warm, open-ended question to encourage them to share naturally. Focus on making a genuine connection."
    },
    "mass_message": {
...
# Il resto del codice Flask rimane identico
@app.route('/')
def index():
...
def test_ai():
    return jsonify({'status': 'OK', 'message': 'S&S No-KYC-Docs Engine is running!', 'model_in_use': 'gemini-2.5-pro'})

def determine_final_strategy(fan_message_lower, situation, submenu):
    situation_data = TASK_STRATEGIES.get(situation)
    if situation_data:
        # Per KYC, userà sempre la strategia generica perché i sottomenù specifici non esistono più
